...
ChatIDArgumentType = Union[Iterable[Union[int, str]], str, int]


def extract_chat_ids(chat_id: ChatIDArgumentType) -> set[int]:
...
class Command(Filter):
    ...
    @classmethod
    async def check_command(cls, message: types.Message, commands, prefixes, ignore_case=True, ignore_mention=False,
                            ignore_caption=True):
...
class CommandStart(Command):
    ...
    def __init__(self,
                 deep_link: Optional[Union[str, typing.Pattern[str]]] = None,
                 encoded: bool = False):
        """
    ...
    async def check(self, message: types.Message):
        ...
        if check and self.deep_link is not None:
            ...
            return {'deep_link': match} if match else False
...
class StateFilter(BoundFilter):
    ...
    async def check(self, obj):
        if '*' in self.states:
            return {'state_ctx': self.dispatcher.current_state()}
        ...
        try:
            ...
            if chat or user:
                ...
                if state in self.states:
                    return {'state_ctx': self.dispatcher.current_state(), 'raw_state': state}
...
class IDFilter(Filter):
    def __init__(self,
        ...
        self.user_id: Optional[set[int]] = None
        self.chat_id: Optional[set[int]] = None
...
class ChatTypeFilter(BoundFilter):
    ...
    async def check(self, obj: Union[Message, CallbackQuery]):
        if isinstance(obj, Message):
            ...
            warnings.warn(f"ChatTypeFilter doesn't support {type(obj)} as input")
            return False
...
class ControlPrefix(BoundFilter):
    """Check deeplink, query.data or query.query for control prefix;
    return payload without prefix."""

    key = 'cprefix'

    def __init__(self, cprefix: str):
    ...
    async def check(self, obj: Union[Message, CallbackQuery, InlineQuery]) -> Union[dict, bool]:
        if isinstance(obj, Message):
            ...
            if command == '/start' and deeplink.startswith(self.cprefix):
                payload = deeplink.removeprefix(self.cprefix)
                return {'payload': payload}
        elif isinstance(obj, CallbackQuery):
            if obj.data.startswith(self.cprefix):
                payload = obj.data.removeprefix(self.cprefix)
                return {'payload': payload}
        elif isinstance(obj, InlineQuery):
            if obj.query.startswith(self.cprefix):
                payload = obj.query.removeprefix(self.cprefix)
                return {'payload': payload}
        return False


class StorageData(BoundFilter):
    """Check if passed dict is subdict of storage data (for current User+Chat)."""

    key = 'sdata'

    def __init__(self, dispatcher, sdata: dict):
        self.dispatcher = dispatcher
        self.sdata = sdata

    def is_subdict(self, main_dict: dict) -> bool:
    ...
    async def check(self, *args) -> bool:
        ...
        return self.is_subdict(storage_data)
