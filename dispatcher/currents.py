...
def get_state_by_name(state_name: str) -> Optional[State]:
...
class CurrentObjects:
    ...
    @classmethod
    def get(cls, ctx_obj: str) -> ContextObj:
        return getattr(cls, ctx_obj)

    @classmethod
    def set_alias(cls, ctx_obj: str, alias: str):
        setattr(cls, alias, cls.get(ctx_obj))
    ...
    @classmethod
    def decorate(cls, async_func) -> callable:
        ...
        async def wrapper(*args, **kwargs):
            ...
            resolved_kwargs = {}
            spec_args = inspect.getfullargspec(async_func).args

            for arg in spec_args:
                if arg in kwargs:
                    resolved_kwargs[arg] = kwargs[arg]
                elif arg in cls.keywords:
                    resolved_kwargs[arg] = await cls.get(arg).get_current()
            return await async_func(**resolved_kwargs)
